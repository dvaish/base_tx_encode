...
class SdGenerator extends Module {
	val io = IO(new Bundle {
        ...
        val sdn = Output(Vec(9, Bool()))
        val csreset = Output(Bool())
    })
    
    // TODO: Check this
    val OK = 1.U 
    val tx_enable_n_2 = ShiftRegister(io.tx_enable, 2, 0.B, 1.B)

    // CSN Generation
    io.csreset := tx_enable_n_2 && (!io.tx_enable)

    val cs_n = Wire(Vec(3, Bool()))
